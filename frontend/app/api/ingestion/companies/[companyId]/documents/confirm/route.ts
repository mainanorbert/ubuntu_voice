...
/**
 * Proxies the post-upload confirmation to the FastAPI backend.
 *
 * The browser calls this AFTER it has PUT the file bytes directly to Supabase
 * using the URLs returned by the /uploads endpoint. The backend HEAD-verifies
 * each object and inserts the document rows.
 */
export async function POST(request: NextRequest, context: RouteContext): Promise<NextResponse> {
  ...
  const url = `${get_backend_base_url()}/api/v1/companies/${encodeURIComponent(companyId)}/documents/confirm`
